...
| 2026-09-21 | 1.4 | **Roteamento @po dos achados do gate CONCERNS (`docs/qa/gates/debt.10-telemetria-marca-atraso-de-input.yml`). Nenhuma mudança de AC nem de Status (continua Done).** Registrado aqui porque nenhuma story Ready abre os arquivos em questão, e esse é o precedente de E41-TEL-002 → `debt.10` (Change Log de `e4.1` v1.4.0). **DEBT10-TST-001 (medium) → STORY NOVA, a criar pelo @sm (`*create-story`, sugestão `debt.11`: "guarda headless da partição de telemetria").** Não cabe em nenhuma story existente. `e4.5` proíbe `client/telemetria.ts` e `src/tools/` (AC 12/14). `e4.3` e a story do codec abrem `determinism.ts`, mas nada de telemetria, e anexar lá misturaria o gate do fio com o da telemetria. **Conferido na fonte, e o @sm precisa levar para o texto:** (a) `src/tools/telemetria.ts:307` chama `main()` incondicionalmente, e `main()` (`:289`) faz `process.exit(1)` sem `process.argv[2]` (`:293`). Importar o módulo de dentro do `sim:check` **mata o processo**. A story precisa de uma guarda de ponto de entrada no CLI (rodar `main()` só quando executado como script), e essa é a única mudança de corpo esperada em `tools/telemetria.ts`. (b) `src/client/telemetria.ts` só toca `localStorage` dentro de `criarTelemetria`/`ler` (`:150`, `:191`) e `document` só no download (`:220`), então é importável em Node com um `localStorage` falso em `globalThis`. (c) A guarda sugerida pelo @qa: `agregar()` sobre um fixture misto commitado (ex. `docs/evidence/telemetria/fixture-misto.json`) afirma que nenhum bloco sai com o n combinado e que "sem carimbo" cai em desconhecido; o coletor com `localStorage` falso afirma o carimbo gravado em `registrar()` e a ausência dele no acúmulo antigo; e as 4 mutações do gate passam a reprovar. **Onde a guarda vive: recomendação do @po, dentro do `sim:check`**, porque é o comando que todo gate já roda, e um script irmão só protege se cada `quality_gate_tools` futuro lembrar de listá-lo. **A seta é decisão do @architect**, e é pré-condição da story: `tools/determinism.ts → tools/telemetria.ts → client/`. A §2.2 (nota de `e4.2`) já registra a dependência mútua de pastas `tools/ ↔ client/` sem ciclo de arquivo. A guarda a tornaria parte do `sim:check`. **Sequência:** `determinism.ts` é disputado pela story do codec e por `e4.3`. `debt.11` começa depois do commit de implementação da que estiver em curso, com escopo conferido por `git show --stat <commit próprio>`. **Prazo:** o do AC 11 desta story, antes da primeira coleta humana usada como evidência de `debt.9` (pré-condição b) ou como baseline de P4.4 (`e4.7`). **DEBT10-DOC-002 (low) → junto em `debt.11`**, que já abre `docs/evidence/telemetria/` para o fixture. A linha `b8e8a41` do README passa a dizer que a janela de atraso 6 sem carimbo vale só para bundle com `b8e8a41` e sem `3de1cfe` (vazia no build publicado, que leva os dois no mesmo push), e a linha `debt.10` ganha a hora (2026-09-21 03:58:44 −03). **DEBT10-COD-003 (low) → junto em `debt.11`, como item opcional:** alinhar `ler()` a `!Number.isFinite`, como `populacaoDe()`, e separar "sem carimbo" de "carimbo inválido" no texto do agregador. Entra porque a guarda nova vai fixar a semântica de "sem carimbo", e deixar os dois critérios divergindo seria fixar a divergência. Se o @sm preferir manter `debt.11` mínima, fica no gate, onde já está registrado com dono @dev (opcional). **DEBT10-INFO-004** fica com @qa (re-gate de `e4.1`), sem ação do @po. | @po |
