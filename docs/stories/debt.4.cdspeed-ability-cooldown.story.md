...
| 2026-07-28 | 1.3 | **QA-001 resolvido**, decisão do usuário: subir o piso para **500ms** (folga real acima dos 450ms, não apenas encostar). `MIN_ABILITY_CD_MS` em `world.ts` corrigida de 400 para 500, com comentário explicando a correção. `architecture.md` §1.4/§3.3 atualizado (número na tabela, no texto do Mecanismo 2 e no exemplo de cálculo do Golem), com nota de correção registrada no próprio documento. `debt.1` (Dev Notes, já `Done`) recebeu nota apontando para esta correção — código daquela story não mudou, só o valor do clamp derivado, aplicado no ponto de consumo por esta story. `debt.6` referenciava só o nome da constante, não o literal — herda 500 automaticamente. Verificado: `tsc` limpo, `sim:check` verde (`golden hash` + `build coverage`, ambos idênticos — o piso segue código inalcançável no roster atual, então a mudança de valor não altera nenhum hash). | @dev |
...
| Arquivo | Mudança |
|---|---|
| `src/sim/world.ts` | `MIN_ABILITY_CD_MS` (nova constante, valor corrigido de 400 para 500 em v1.3 — ver Change Log); `castCommand` aplica `cdSpeed` com piso |
