...
### Reviewed By: Quinn (@qa) — gate `docs/qa/gates/debt.14-guarda-da-sala-por-variante-e-por-assento.yml`

**Gate: CONCERNS → Status Done.** Revisões avaliadas: `17ecf5c` e `f9b0312` (implementação). O pai é `6d525df`, só docs, e o código-base é o de `debt.12` (`5766137`). `2129004` (@sm, e4.10) fica entre os dois. O registro é `96eae5a`, que toca só esta story. `git diff f9b0312 HEAD -- src` está vazio. É commit local, não publicado (`merge-base --is-ancestor f9b0312 origin/master` rc=1).

**Verificação independente (tudo refeito do zero; worktree descartável de `17ecf5c^`; mutações em cópias de `git archive f9b0312`, nunca na árvore compartilhada):**

| AC | Veredito | Evidência |
|---|---|---|
| 1 | MET | `npm run check` rc=0. `npm run build` rc=0 (53.27 kB) |
| 2 | MET | `sim:check` rc=0, golden hash ✓. O `diff` da saída inteira contra `6d525df` tem só `65,66c65,66` (contadores de `assentos` 21→28/12→17/76→119 e `autoridade` 176→198, ✓ mantido) e `67a68,70` (`por variante`, `cobertura`, `bordas`). `sala pura`, `bo5`, `flush` e `negativos` são byte a byte iguais, e nada fora do bloco muda |
| 3 | MET | 6/6 casos (build, buildPadrao e pronto em `builds`; compra, trocaDeBuild e pronto na 2ª loja, ouro [9,9]). Cada um tem oráculo `aplicar()`, recusa com exatamente 1 `{t:'erro'}` ao remetente, `partida` ref-igual, log intacto e controle positivo do dono. Q3 e as 5 isenções reprovam, cada uma pela própria variante. O1 (checagem só no assento 0) reprova, porque os casos vão nas duas direções |
| 4 | MET, com ressalva | `conferirCobertura()` roda em todo `conduzir()`: 52 transições e 6 rodadas. Q5, Q6 e Q7 reprovam pela linha nova. O5 (2 `rodadaFim` por assento) reprova. A contagem por sala no lugar da por assento (O4) perde quase tudo, então a contagem por assento é o que dá dente. **Ressalva:** o passo final (sala → `encerrada`) fica fora da regra da visão (DEBT14-TST-001) |
| 5 | MET | (i) Snap final no tick 30, `over:false`, antes de 2 `rodadaFim` com `ticks` 30. Q8 e O9 (sem snap final) reprovam. (ii) 1 `{t:'prazo'}` com 5 000 ms restantes. Q12 reprova, e O8 (prazo cheio, 30 000) também: a guarda é de valor, e não só de presença. (iii) Queda em `builds` a 2 s e na loja abrindo a pausa sozinha, com W.O. `buildPadrao`/`pronto`. Q13 reprova nos dois |
| 6 | MET | As 12 mutações foram reaplicadas, todas com rc=1, cada uma por uma linha `✗ sala debt.14`/`(debt.14 AC 4)`. O controle sem mutação deu rc=0 |
| 7 | MET | `git show --stat`: `17ecf5c` e `f9b0312` tocam só `src/tools/determinism.ts` (251/2 e 1/1). Os hunks ficam entre o cabeçalho `sala pura` (1589) e o fim de `guardaSala()`. Nenhum hunk nos imports nem no bloco do codec. `sala.ts`, `codec.ts` e `package.json` não foram tocados. O `a.tar` não está em nenhum commit (`show --name-only`, `ls-tree -r` e `log --all -- '*a.tar'` vazios) |
| 8 | MET | A base é `debt.12` `5766137`. As conferências novas contam por tipo. Simulei a e4.10 (E1/E2: `{t:'evento'}` depois de toda visão, público e privado): zero ✗ de debt.14. Só a lista completa da e4.3 no W.O. quebra, e a e4.10 já tem a tarefa de atualizá-la |

**Mutações próprias (24):** O1–O9 (direção do assento, par do controle positivo, cobertura desligada, contagem por sala, rodadaFim duplicado, pré-condição da loja, visão final, prazo cheio, snap final no teto), F1–F3 (correção proposta de TST-001), D1–D3 (exclusão do draft) e E1–E2 (simulação da e4.10). Os detalhes estão no gate.

**Perguntas do lead:**
1. **`aplicar()` como oráculo**: é sólido. É a mesma função que `decidir()` chama (`sala.ts:510`). Quando o oráculo recusa, o caso vira ✗, e não é pulado. O dente anti-vácuo é o controle positivo no mesmo estado: O2a (a sala recusa toda `trocaDeBuild` de cliente) reprova só por ele, e O2b (O2a com o controle desligado) passa verde.
2. **Exclusão emergente do draft**: é robusta para o draft. D2 (o draft fica `jogando`) reativa a regra sozinha, e D1 (a queda em builds regride para `aguardando`) é pega por `bordas`. O efeito colateral é que `encerrada` também sai, e a visão final fica sem cobertura (DEBT14-TST-001).
3. **Depois da e4.10**: continua verde, e verifiquei por execução com E1/E2.

**Achados:**
- **DEBT14-TST-001 (medium, tests; @po roteia, e a e4.10 é o dono natural):** a condição `s.fase === 'jogando'` exclui o passo final da partida. Sem visão final (O7), ou com ela só ao assento 0 (O7b), o resultado é rc=0. O AC 4 v1.1 foi seguido ao pé da letra, então não é defeito do @dev. A correção é `s.fase !== 'aguardando'`: F1 fica verde, e F2/F3 pegam O7b/O7. Tem de entrar antes de a e4.4 ligar a sala ao socket.
- **DEBT14-TST-002 (low, tests; @dev, opcional, na próxima story do bloco):** a linha `cobertura` não tem piso. Com `conferirCobertura()` desligada, ela imprime `✓ 0 transição(ões) · 0 rodada(s)` com rc=0 (O3, O3b). A correção é um piso nos contadores dentro de `problemas`.

**Observações:** o Status veio "Ready for Review" (o canônico é InReview), e a transição foi registrada como Ready for Review → Done. A e4.10 ainda diz, na linha 505, que debt.14 está "sem commit de implementação" (é texto de estado, fica para o @po). CodeRabbit não roda nesta máquina. Worktree e cópias removidos, e `git status --short src` saiu vazio.
...
| 2026-09-21 | 1.3 | **Gate do @qa: CONCERNS. Status Ready for Review → Done.** Gate em `docs/qa/gates/debt.14-guarda-da-sala-por-variante-e-por-assento.yml`. Os 8 ACs estão MET. check, sim:check e build com rc=0, e golden hash idêntico. O diff da saída contra `17ecf5c^` tem só os contadores de `assentos`/`autoridade` e as 3 linhas novas. As 12 mutações da story foram reaplicadas, todas com rc=1, e rodei 24 minhas. E43-TST-001/002/003 estão fechados. Dois achados: DEBT14-TST-001 (medium, visão final sem cobertura por assento, porque o passo final termina em `encerrada`; @po roteia) e DEBT14-TST-002 (low, linha `cobertura` sem piso; @dev, opcional). Só QA Results, Status e esta linha foram tocados. | Quinn (@qa) |
| 2026-09-21 | 1.4 | **Roteamento dos achados do gate (`257b9ef`) pelo @po. Status permanece Done, e AC e QA Results não mudam.** `DEBT14-TST-001` (medium: `conferirCobertura()` pula o passo final porque a sala termina em `encerrada`) e `DEBT14-TST-002` (low: a linha `cobertura` não tem piso) entram **os dois** em `docs/stories/e4.10.evento-partida-no-fio-por-assento.story.md` v1.3.0, **AC 13**. Lá ficam a condição `s.partida !== antes.partida && s.fase !== 'aguardando'`, o piso (transições `>= 53`, rodadas fechadas `>= 6`, com mensagem em `problemas`) e as mutações MC1 (sem visão final), MC2 (visão final só ao assento 0), MC3 (`conferirCobertura()` desligada) e MC4 (condição revertida), todas com rc ≠ 0 no par `npm run check && npm run sim:check`. A `e4.10` é a próxima story a abrir o bloco `sala pura` e vem antes de a `e4.4` ligar a sala ao socket. Medido pelo @po numa cópia descartável de `257b9ef` (os números estão na linha v1.3.0 da `e4.10`). | Pax (@po) |
