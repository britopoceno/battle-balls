...
| 2026-09-21 | 1.6 | **Deltas de AC registrados (DEBT12-REQ-001, gate `docs/qa/gates/debt.12-guarda-do-fio-tabela-e-fixture-congelada.yml`, `34f6af5`) e achados LOW de teste encaminhados. Status permanece Done. O texto dos ACs não foi editado e QA Results não foi tocado** (precedentes: `debt.11` v1.5 e `e4.3` v1.9.0). **(1) Deltas entregues em `5766137` por orientação do @architect (`188998e`, §11.6.1 emendada e §11.6.2), e já julgados em escopo pelo gate.** Os ACs passam a ser lidos assim. **(a) AC 5, 5º item da guarda e forma da entrada.** Cada entrada de `FIO_CONGELADO` é `{ versao, snap, variantes }`, e a v1 tem `variantes: 'erro,ping,prazo,rodadaFim,rodadaInicio,sala,snap,visao'`. O item 5 confere que a lista atual de variantes de `DoServidor` é igual a `última.variantes`, com a mesma instrução de mudança do item 1. A lista atual vem das chaves de `amostras` da linha `não-snap` mais `'snap'`, em `sort()` padrão e unida por vírgula (`determinism.ts:1567`, com `variantesNaoSnap = Object.keys(amostras)` em `:1406`). Ela não vem de `T_DO_SERVIDOR`, que é privada de `codec.ts`, arquivo proibido pelo AC 7 (§11.6.2). **(b) AC 6, mutação M-VARIANTE**, a 11ª do AC 6: uma variante a mais na lista atual, sem entrada nova, tem de dar rc ≠ 0. Saiu rc=1 (Dev Agent Record), e a P9 do gate mostrou que o item 5 é a única defesa contra ela. **(c) AC 9, versão errada relativa.** O caso de M-9 é `` [`versao ${VERSAO_DO_FIO + 1}`, salaCom('versao', VERSAO_DO_FIO + 1), VERSAO_DO_FIO + 1] `` (`:1335`), e não o literal `2`. O texto da linha `versão fio` cita `${VERSAO_DO_FIO + 1}` (`:1403`). Com a v1, a saída é byte a byte a mesma. Com `VERSAO_DO_FIO = 2` e a entrada v2, a linha fica ✓ com `3`, e por isso `e4.10` não reabre esse caso. O contrafactual da story passa de 14 para 15 mutações. **(d) Ressalva medida pelo @qa (DEBT12-TST-002), que acompanha o item 5:** o item 5 só discrimina quando `npm run check` roda junto com `npm run sim:check`. O `sim:check` roda o `.ts` direto no Node, sem checagem de tipos, e o elo `DoServidor` ↔ `T_DO_SERVIDOR` ↔ chaves de `amostras` é o `satisfies`, que só o `tsc` confere. Uma variante nova sem amostra deixa o `sim:check` com rc=0, e só o `tsc` reprova (rc=2), nos casos Q2a e Q2b do gate. A prova do item 5 é sempre o PAR `npm run check && npm run sim:check`, e nenhum CI roda os dois (`deploy-pages.yml` roda só `vite build`). A ressalva já está em `e4.10` desde a v1.1.0 (`bbef157`), no quality_gate_tools, no AC 1 e na mutação ME6b. **(2) Achados LOW de teste, opcionais, para a próxima story que abrir o bloco correspondente de `determinism.ts`. Nenhum dos dois entra em `e4.10`.** **DEBT12-TST-001, canário com arredondamento local.** O item 4 compara `Math.floor(v*100)/100` e `Math.ceil` locais (`:1555-1558`) com `Math.round`, e os locais divergem de `pisoQ`/`tetoQ` do codec justamente na borda de ponto flutuante. A correção é ancorar o canário na saída codificada, sem abrir `codec.ts`: o ultCharge codificado (`s[6][0][6]` da última entrada) tem de ser diferente de `Math.round(ultCharge*100)/100`, e o restante codificado (`s[6][0][7]`) diferente de `Math.round(restante*100)/100`. Mutação de aceite: a P6b do gate (ultCharge 0.29 na amostra, texto regenerado) sai rc=1. *Conferido pelo @po numa cópia descartável de `bbef157` (Node 24.13.1), importando o codec real:* com 55.559, o codificado é `55.55` e o arredondado `55.56`, e tanto o canário de hoje quanto o ancorado dão ✓. Com 0.29, o piso local é `0.28`, mas o codificado é `0.29`, igual ao arredondado. O canário de hoje dá ✓, o que está errado, e o ancorado dá ✗, o que está certo. No texto da v1, `s[6][0][6] = 55.55` e `s[6][0][7] = 183.35`. **DEBT12-TST-003, tabela do parser sem trava de tamanho.** `numericos` (`:1082`), `bits` (`:1102`) e `descartes` (`:1129`) têm 4, 14 e 16 linhas, e o tamanho só aparece impresso na linha `parser` (`:1147`). Não existe asserção de `length`. A correção é travar os três tamanhos contra literais comentados, no padrão de `POPULACOES_FIXTURE` da `debt.13`, com uma mensagem que mande conferir `finito()` e `bit()` em `codec.ts`. Mutações de aceite: a P1 e a P2 do gate saem rc=1. *Conferido pelo @po:* numa cópia de `bbef157`, a linha `parser` diz `4 numéricos × 4 = 16 e 14 campos 0|1 × 4 = 56 → null · tick/extra descartados em 16 formas`, e o `grep` não acha `numericos.length`, `bits.length` nem `descartes.length` fora do texto dessa linha. **Por que nenhum dos dois vai em `e4.10`:** `e4.10` está Ready (GO 8/10, `bbef157`) e abre o bloco do codec só para a amostra `evento` em `amostras` e para a entrada 2 de `FIO_CONGELADO`, além do bloco `sala pura`. O AC 12 dela fecha o escopo de `determinism.ts` nesses três pontos, e o AC 2 declara exatamente quatro linhas de saída que mudam. TST-003 abre `guardaParser`, que `e4.10` não toca. TST-001 muda o item 4 de `guardaFioCongelado` e traz mais uma mutação de aceite para o gate, numa story que já leva nove mutações e uma subida de protocolo. Nos dois casos o risco é baixo. Ele só aparece se alguém editar o próprio teste ou trocar a amostra por um valor de borda, e trocar a amostra já é mudança de protocolo. **Donos naturais:** TST-003 vai com a próxima story que mudar `DoCliente` ou a seção ENTRADA de `codec.ts`. TST-001 vai com a primeira subida que mudar o layout do `snap`, que é a mesma que, pela errata DEBT12-ARC-001 (§11.6.1, `00c628d`), troca `'shield'` por um kind válido em `AMOSTRA_DO_FIO`. É o momento em que alguém vai mexer no valor da amostra. Hoje nenhuma story Ready ou Draft abre `guardaParser` ou `guardaFioCongelado`. Das que citam `parseDoCliente`, só `e4.4` é Draft, e ela só usa o parser e proíbe `src/net/`. `e4.10` v1.2.0 retira a menção a TST-001 como "opcional" que a v1.1.0 de lá tinha feito. | Pax (@po) |
