...
|---|---|---|---|
| 2026-08-16 | 2.1.0 | QA Gate PASS — Status: InReview → Done — 8/8 ACs MET, varredura reproduzida independentemente, duplo-KO provado pelos números; follow-up QA-D8-01 (asserção estrutural anti-re-baseline) registrado no gate. | @qa |
...
**Gate: PASS** · Quinn (@qa) · 2026-08-16 · gate completo em `docs/qa/gates/debt.8-tie-path-golden-seed.yml`

**Anti-self-review observado** (o @dev desta story foi esta mesma sessão): a verificação rodou em agentes independentes. O verificador escreveu o próprio scanner do zero, varreu 200-379 e reproduziu a alegação número por número — nenhum empate antes da 379, e na 379 exatamente `a2bb5327`/3831/`-1`, idêntico em dupla execução. O juiz adversarial provou a classificação "duplo-KO real, não default de inicialização" pelos números pinados + código de `checkEnd` (a armadilha do `winner = -1` inicial que o @po flagou não morde: saída por default pinaria `ticks = 10800`, não 3831). As 10 entradas golden pré-existentes intactas dígito a dígito; os dois comentários obsoletos da seed 11 corrigidos com precisão; escopo cirúrgico nos dois commits.

**PASS limpo, com um follow-up barato registrado (QA-D8-01, low, não bloqueia):** a linha pinada protege contra regressão de motor, mas não contra um RE-BASELINE futuro re-gravá-la em silêncio — a próxima story que tocar `determinism.ts` deve considerar a asserção estrutural de uma linha (`BASELINE` precisa conter pelo menos um `winner === -1`) para a tabela em si não poder perder a cobertura de novo.
