...
| 2026-09-21 | 1.4 | **Roteamento @po dos achados LOW do gate PASS (`f5d4713`). Status permanece Done. Nenhum AC muda de critério, e QA Results não foi tocado.** **Consequência do fechamento:** a cláusula provisória de coleta saiu de `e4.7` (v1.7.0) e de `debt.9` (v1.4). O corte de build de `debt.10` continua valendo nas duas, porque o Pages publicado (`44311f6`) não grava carimbo. **`DEBT13-DOC-002`, registrado aqui sem reabrir a story:** o AC 5 diz que "só a checagem de quantas são desconhecidas" pega Q13b, e isso subestima a guarda. O gate mediu que a posição (`slice(-3)`, X5) e a soma da R6 (X6) também pegam Q13b, cada uma sozinha. É inofensivo, porque o erro é a favor da segurança, e a regra "não afrouxar para pelo menos uma" continua certa. O texto do AC 5 fica como está, por ser o critério que o gate julgou. Quem tocar no comentário da M2 em `guarda-telemetria.ts` usa a formulação do gate. **`DEBT13-DOC-001` fica no gate, sem story:** a lista de "Suspeitos" do `throw` da telemetria em `determinism.ts` (`HEAD`, bloco `problemasTelemetria`) não cita o carimbo com valor errado. `debt.12` não é casa natural: o AC 7 de lá abre só `determinism.ts`, mas a story trabalha no bloco do codec, e não no `throw` da telemetria. Hoje nenhuma story do plano abre esse `throw`. Quem abrir leva o achado junto. **Fatos conferidos:** o `throw` está em `git show HEAD:src/tools/determinism.ts`, logo depois do da BOT-001. O escopo de `debt.12` está no AC 7 e na v1.2 do Change Log de lá ("mesmo bloco do codec"). | Pax (@po) |
