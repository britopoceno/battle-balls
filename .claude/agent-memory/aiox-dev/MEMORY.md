...
- [Golden hash / replay não provam correção](project_golden_hash_cobertura.md) — hash e replay medem "nada mudou"; hash quantizado ≠ bit a bit; mutação de server/ só a partida real pega.
...
- [Cópia descartável por git archive](project_git_archive_copia_descartavel.md) — `-o` absoluto; tar com C: falha; árvore não commitada: cp + junction via PowerShell New-Item, .Delete().
- [Verificação de navegador por CDP](project_verificacao_navegador_cdp.md) — sem claude-in-chrome, Chrome headless por CDP; 5179 pode estar ocupada; telemetria vaza entre abas.
